...
- **`examples/short-selling.ts`** — a runnable, commented walkthrough of the full flow (margin check → shortability check → review → short → cover). Dry-run by default; `--place` is required to submit anything.
